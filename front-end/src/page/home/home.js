...
function Home() {
  ...
  const getLaptopOffice = () => {
    Instance.get("/laptop-vanphong")
      .then((res) => {
        setLaptopOffice(res.data);
      })
      .catch((err) => {
        console.log(err);
      });
  ...
  const sortHotSaleProducts = (data) => {
    const sortedProducts = [...data].sort(
      (a, b) => b.prod_percent - a.prod_percent
    );
    ...
    return (
      ...
      >
        ...
        >
          ...
          <span
            className="text-[#1d1d1d] text-[16px] 
                line-through"
          >
            {formatPriceWithCommas(product.cost)} đ
          </span>
          <span
            className="text-[#c8191f] text-[21px] 
                font-bold"
          >
  ...
  return (
    <div className="bg-[#f0f0f0]">
      <div className={`${!isHiddenAutoCpl ? "hidden" : ""} banner w-full`}>
        <Carousel autoplay pauseOnDotsHover pauseOnHover draggable>
          <div>
            <img
              className="min-h-[406.58px]"
              src="http://localhost:8000/upload/banner2.png"
            ></img>
          </div>
          <div>
            <img
              className="min-h-[406.58px]"
              src="http://localhost:8000/upload/banner1.jpg"
            ></img>
          </div>
          <div>
            <img
              className="min-h-[406.58px]"
              src="http://localhost:8000/upload/banner3.png"
            ></img>
          </div>
        </Carousel>
      </div>
